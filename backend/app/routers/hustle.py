"""
Hustle Score API Router - Endpoints for querying student reputation scores
"""
...
class ScoreResponse(BaseModel):
    """Response model for student score."""
    wallet_address: str
    score: int
    initialized: bool
...
async def get_student_score(wallet_address: str):
    """
    Get Hustle Score for a student by wallet address.
    ...
    try:
        ...
        return ScoreResponse(
            ...
            score=score,
            initialized=score > 0  # If score > 0, definitely initialized
        )
...
async def initialize_student(wallet_address: str):
    """
    Manually initialize a student's Hustle Score (creates box storage).
    Usually called automatically when first reputation points are awarded.
    """
    try:
...
async def add_reputation(wallet_address: str, points: int = 10):
    """
    Manually add reputation points to a student.
    (Admin endpoint - in production, secure this with proper auth)
    """
    try:
