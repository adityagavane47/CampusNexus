#!/usr/bin/env python
"""Direct deployment using compiled TEAL and algosdk."""
import logging
import os
import json
import base64
...
def deploy_app_bare(algod_client, sender_address, sender_sk, approval_program, clear_program, global_schema, local_schema):
    """Deploy an application with bare create (no ABI method)."""
...
def deploy_app_with_abi_create(algod_client, sender_address, sender_sk, approval_program, clear_program, global_schema, local_schema):
    """Deploy an application with ABI create method."""
    params = algod_client.suggested_params()
    
    # Build ABI method selector for create()string
...
def main():
    ...
    # Read compiled TEAL programs for MilestoneEscrow
    ...
    # Deploy MilestoneEscrow (bare create - no ABI method)
    ...
    escrow_app_id, escrow_app_address = deploy_app_bare(
        ...
        escrow_clear_bytes,
        transaction.StateSchema(num_uints=3, num_byte_slices=2),  # Global schema
        transaction.StateSchema(num_uints=0, num_byte_slices=0)   # Local schema
    )
    logger.info(f"✅ Deployed MilestoneEscrow with app_id: {escrow_app_id}")
    
    # Read compiled TEAL programs for HustleScore
    ...
    # Deploy HustleScore (with ABI create method)
    ...
    hustle_app_id, hustle_app_address = deploy_app_with_abi_create(
        ...
        hustle_clear_bytes,
        transaction.StateSchema(num_uints=1, num_byte_slices=1),  # Global schema
        transaction.StateSchema(num_uints=0, num_byte_slices=0)   # Local schema
    )
    logger.info(f"✅ Deployed HustleScore with app_id: {hustle_app_id}")
    
    # Write deployed contracts to JSON
    deployed = {
        "network": "testnet",
        "contracts": {
            "MilestoneEscrow": {
